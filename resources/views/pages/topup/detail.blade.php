	...
	<div class="row g-4">
		...
		<div class="col-lg-8">
			<form action="#" method="POST" id="orderForm">
				...
				<div class="card border-0 shadow-sm mb-4" style="background: var(--card-bg);">
					<div class="card-body p-4">
						...
						<div class="row g-3">
							@php
								$nominals = [
									['name' => 'Weekly Diamond Pass', 'price_str' => 'Rp 28.500', 'price_int' => 28500],
									['name' => '86 Diamonds', 'price_str' => 'Rp 20.000', 'price_int' => 20000],
							...
							<div class="col-6 col-md-4">
								<div class="card h-100 nominal-card p-3 text-center"
									onclick="selectNominal(this, '{{ $nom['price_str'] }}', {{ $nom['price_int'] }})">

									<div class="active-icon"><i class="fas fa-check"></i></div>
									<div class="fw-bold mb-1 text-body item-name">{{ $nom['name'] }}</div>
									<small class="text-primary">{{ $nom['price_str'] }}</small>

									<input type="radio" name="nominal_name" value="{{ $nom['name'] }}" class="d-none">
								</div>
							</div>
							@endforeach
							<input type="hidden" id="realPrice" name="real_price" value="">
						</div>
					</div>
				</div>
				...
				{{-- <button type="button" class="btn btn-primary w-100 py-3 fw-bold fs-5 shadow-lg rounded-pill mb-5" id="btnSubmit"> --}}
...
@push('scripts')
@push('scripts')
	{{-- <script id= "midtrans-script" src="https://api.midtrans.com/v2/assets/js/midtrans-new-3ds.min.js" data-environment="<production|sandbox>" data-client-key="{{ env('MIDTRANS_CLIENT_KEY') }}" type="text/javascript"></script> --}}
...
<script>
	// Logic Nominal
	function selectNominal(element, priceStr, priceInt) {
		$('.nominal-card').removeClass('active');
		$(element).addClass('active');
		$(element).find('input[type="radio"]').prop('checked', true);

		// Update Tampilan Harga di Payment Section
		$('.payment-price').text(priceStr);

		// Simpan harga asli (integer) ke input hidden
		$('#realPrice').val(priceInt);
	}

	// Logic Payment UI Only (Midtrans Snap akan handle metode pembayaran sebenarnya)
	...
	// var payButton = document.getElementById('btnSubmit');
	// payButton.addEventListener('click', function () {
	// 	// Trigger snap popup. @TODO: Replace TRANSACTION_TOKEN_HERE with your transaction token.
	// 	// Also, use the embedId that you defined in the div above, here.
	// 	const id = "{{ env('MIDTRANS_CLIENT_KEY') }}"
	// 	// console.log(id);

	// 	window.snap.embed("{{ env('MIDTRANS_CLIENT_KEY') }}", {
	...
	$('#orderForm').on('submit', function(e) {
		...
		let zoneId    = $('input[placeholder="(1234)"]').val();
		let itemName  = $('input[name="nominal_name"]:checked').val();
		let nominal   = $('#realPrice').val(); // Ambil harga integer
		let whatsapp  = $('#whatsappNumber').val();

		// Validasi Sederhana
		if(!userId || !zoneId) { alert('Lengkapi Data Akun!'); return; }
		if(!itemName) { alert('Pilih Nominal!'); return; }
		...
		$.ajax({
			...
			data: {
				...
				user_id: userId,
				zone_id: zoneId,
				nominal: nominal, // Harga (20000)
				item_name: itemName,
				whatsapp: whatsapp
			},
			success: function(response) {
				// Jika sukses dapat token, munculkan Popup Snap
				window.snap.pay(response.snap_token, {
					onSuccess: function(result){
						alert("Pembayaran Berhasil!");
						console.log(result);
						window.location.href = "/"; // Redirect ke home/history
					},
					...
					onError: function(result){
						...
						console.log(result);
						btn.prop('disabled', false).html('Beli Sekarang');
					},
					onClose: function(){
