			...
			<div class="col-md-3">
				<div class="card border-0 shadow-sm p-3">
					<div class="d-flex align-items-center">
						...
						<div><span class="text-muted small text-uppercase fw-bold">Gangguan</span><h4 class="fw-bold mb-0">{{ $gangguanCount }}</h4></div>
					</div>
				</div>
			</div>
		</div>
...
@push('scripts')
	...
	<script>
		...
		$(document).on('click', '.btn-edit', async function() {
			...
			{status, data: {data}, data: {meta}} = await postRequest("{{route('admin.products.brands.form')}}", {brand_id: id});
